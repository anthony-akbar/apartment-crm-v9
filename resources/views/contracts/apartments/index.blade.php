...
@section('content')
    <h2 class="intro-y text-lg font-medium mt-10">Договора</h2>

    <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
        <a href="{{ route('contracts.apartments.create') }}" class="btn btn-primary shadow-md mr-2">Новый договор</a>
        <div class="dropdown">
        ...
        <div class="hidden md:block mx-auto text-slate-500">Showing {{ $aptContracts->count() }} contracts</div>
        <div class="w-full sm:w-auto mt-3 sm:mt-0 sm:ml-auto md:ml-0">
            <div class="w-56 relative text-slate-500">
                <input id="searchInput" type="text" class="form-control w-56 box pr-10" placeholder="Search...">
    ...
    <div id="apartmentsTable" class="grid grid-cols-12 gap-6 mt-5">
        @include('contracts.apartments.table')
    </div>
...
@section('script')
    <script type="text/javascript">
        ...
        $('#searchInput').change(function () {
            ...
            $.ajax({
                url: '{{ route('contracts.apartments.find') }}',
                data: {
                    'data': $(this).val(),
                },
                type: 'GET',
                success: function (data) {
                    $('#apartmentsTable').html(data)
                    console.log(data)
                }
            })
        })
